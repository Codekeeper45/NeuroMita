        ...
        *,
        ...
        return {
            ...
            "content": (
                ...
                f"{safe_rules}\n"
                "\n[Action consistency]\n"
                "Your personality and relationship stats decide whether you accept a request. "
                "If you refuse, say so; if you agree to act now, include the executable action "
                "in the SAME response segment. Spoken agreement alone never moves your body. "
                "Instructions not to disclose technical commands mean keep identifiers out of "
                "spoken text, not leave action fields empty.\n"
                "For following, prefer actor.set_movement_mode with payload {\"mode\":\"follow\"} "
                "IF it is listed above. Legacy movement_modes is only a fallback for runtimes "
                "without that intent. Do not combine conflicting movement channels.\n"
                "For your own sleep use actor.sleep with {} IF listed above. allow_sleep controls "
                "the PLAYER's permission; emotions sleep/halfsleep only change your face. "
                "Never commit sleep before the runtime reports the required bed state.\n"
                "For furniture use interactions with an exact currently available identifier. "
                "Respect occupancy, distance and current interaction state. If navigation is needed, "
                "use only a listed movement target and wait for arrival before interacting; putting "
                "walk and sit in consecutive text segments does not wait for navigation. "
                "Never invent coordinates, targets, or successful completion. If already seated "
                "and asked to follow, exit using the runtime's current interaction exit identifier.\n"
                "Before returning JSON, check that every accepted immediate physical action has "
                "its matching nonempty action field. If unavailable, explain the limitation "
                "instead of claiming you performed it.\n"
                "[/Action consistency]\n"
                "[/Unity Intent Contract]"
            ),
        }
